...
@interface JKMenuDropView () <JKMenuDetailViewDelegate>
...
@implementation JKMenuDropView

+(instancetype)JKMenuDropViewWithSize:(CGSize)size{
    JKMenuDropView * view = [[[NSBundle mainBundle] loadNibNamed:@"JKMenuDropView" owner:nil options:nil]lastObject];
...
- (void)setDelegate:(id<JKMenuDropViewDelegate>)delegate{
    _delegate = delegate;
    NSInteger totolCol = [delegate numOfColInJKMenuView];
    NSLog(@"---%zd",totolCol);
...
- (void)setMenuTitles:(NSArray *)menuTitles{
    ...
    if (menuTitles.count) {
        ...
        for (int i = 0; i < menuTitles.count; i ++) {
            ...
            [menuBtn setTitleColor:RGB(73, 141, 222) forState:UIControlStateSelected];
            [menuBtn setImage:[UIImage imageNamed:@"down"] forState:UIControlStateNormal];
            [menuBtn setImage:[UIImage imageNamed:@"up"] forState:UIControlStateSelected];
...
- (void)menuBtnAction:(UIButton *)button{
    ...
    for (UIView * subView in [UIApplication sharedApplication].keyWindow.subviews) {
        if ([subView isKindOfClass:[JKMenuDetailView class]]) {
            [subView removeFromSuperview];
        }
    }
    ...
    JKMenuDetailView * maskView = [JKMenuDetailView JKMenuDetailViewWithSize:maskViewSize];
    ...
    if ([self.delegate respondsToSelector:@selector(JKMenuItemTypeForRowsAtMenuBtnIndex:)]) {
        if ([self.delegate JKMenuItemTypeForRowsAtMenuBtnIndex:btnIndex] == JKMenuItemTypeSingle) {
            maskView.menuType = JKMenuDropTypeSingleCol;
            maskView.menuBtnTitle = btnTitle;
        } else {
            maskView.leftTabIndex = self.leftTabIndex;
            maskView.seletedLeftString = self.leftSelectedString;
            maskView.menuBtnTitle = btnTitle;
            maskView.menuType = JKMenuDropTypeTwoCol;
        }
    } else {
        maskView.menuType = JKMenuDropTypeSingleCol;
        maskView.menuBtnTitle = btnTitle;
    ...
    if ([self.delegate respondsToSelector:@selector(JKMenuDataSourceAtMenuBtnIndex:)]) {
        NSArray * dataArray = [self.delegate JKMenuDataSourceAtMenuBtnIndex:btnIndex];
...
#pragma mark - JKMenuDetailViewDelegate 改变menuBtn的选中状态
/************************JKMenuDetailViewDelegate代理部分****************************/
...
- (void)selectedMenuItemWitnMenuIndex:(NSInteger)menuIndex dataModel:(JKMenuDataModel *)model indexPath:(NSIndexPath *)indexPath{
    ...
    if ([self.delegate respondsToSelector:@selector(JKMenuDidSelectedItemWithMenuBtnIndex:menuItem: leftMenuItemString:)]) {
        [self.delegate JKMenuDidSelectedItemWithMenuBtnIndex:menuIndex menuItem:menuItem leftMenuItemString:@"Just exist singleTable"];
    }
}
...
- (void)selectedLeftTableWithMenuIndex:(NSInteger)menuIndex leftDataModel:(JKMenuDataModel *)leftDataModel leftTableIndexPath:(NSIndexPath *)leftIndexPath rightDataModel:(JKMenuRightTableDataModel *)rightDataModel rightDataIndexPath:(NSIndexPath *)rightIndexPath{
    ...
    if ([self.delegate respondsToSelector:@selector(JKMenuDidSelectedItemWithMenuBtnIndex:menuItem: leftMenuItemString:)]) {
        [self.delegate JKMenuDidSelectedItemWithMenuBtnIndex:menuIndex menuItem:menuItem leftMenuItemString:self.leftSelectedString];
    }
...
/************************JKMenuDetailViewDelegate代理部分结束****************************/
- (void)layoutSubviews{
