...
@interface JKMenuDetailView () <UITableViewDelegate,UITableViewDataSource>
/**本界面的尺寸*/
...
@implementation JKMenuDetailView
+ (instancetype)JKMenuDetailViewWithSize:(CGSize)size{
    JKMenuDetailView * maskView = [[[NSBundle mainBundle] loadNibNamed:@"JKMenuDetailView" owner:nil options:nil]lastObject];
...
- (void)setMenuType:(JKMenuDropType)menuType{
    _menuType = menuType;
    if (menuType == JKMenuDropTypeSingleCol) {
...
- (void)setDataArray:(NSArray *)dataArray{
    ...
    /**只有一个table的时候*/
    if (self.menuType == JKMenuDropTypeSingleCol) {
        for (int i = 0; i < dataArray.count; i++) {
            JKMenuDataModel * model = [[JKMenuDataModel alloc] init];
    ...
    /**两个视图的时候*/
        ...
        JKMenuDataModel * model = (JKMenuDataModel*)self.dataSource[self.leftTabIndex];
        for (int i = 0; i < model.rightTableData.count; i++) {
            JKMenuRightTableDataModel * rightDataModel = [[JKMenuRightTableDataModel alloc] init];
...
- (UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (self.menuType == JKMenuDropTypeSingleCol) {
        ...
        if (self.dataSource.count) {
            JKMenuDataModel * model = self.dataSource[indexPath.row];
            cell.textLabel.text = model.title;
            UIColor * color = model.isSelectedItem ? RGB(73, 141, 222) : [UIColor blackColor];
        ...
        if (self.rightDataSource.count) {
            JKMenuRightTableDataModel * rightModel = self.rightDataSource[indexPath.row];
            UIColor * color = rightModel.isSelectedItem ? RGB(73, 141, 222) : [UIColor blackColor];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (self.menuType == JKMenuDropTypeSingleCol) {
        ...
        if (tableView == self.leftTableView) {
            for (JKMenuDataModel * model in self.dataSource) {
            ...
            JKMenuDataModel * lastSelectedModel = self.dataSource[self.leftIndexPath.row];
            JKMenuRightTableDataModel * rightTableDataModel = self.rightDataSource[indexPath.row];
...
- (void)changeMainViewWithAnimateClickTheSameBtn:(BOOL)isSame{
    if (isSame == NO) {
        ...
        if (self.menuType ==  JKMenuDropTypeSingleCol) {
            ...
            JKMenuDataModel * model = (JKMenuDataModel*)[self.dataSource firstObject];
...
- (void)touchesBegan:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event{
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:@"kSetBackTableViewContentInsetNSNotification" object:nil];
