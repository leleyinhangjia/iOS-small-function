...
- (NSString *)videoPath
{
    return [PPFolderManager createCacheFilePath:kVideoPath];
}
#pragma mark == 获取视频帧

- (void)getThumbImageOfVideo:(NSURL *)videoUrl forTimes:(NSArray<NSValue *> *)requestedTimes complete:(PCompleteBlock)complete
{
...
- (void)compositionVideoWithImage:(NSArray<UIImage *> *)images videoName:(NSString *)videoName success:(SuccessBlcok _Nullable)successBlcok
{
    ...
    //存在该文件
    if ([PPFolderManager fileExistsAtPath:outPutFilePath]) {
        [PPFolderManager clearCachesWithFilePath:outPutFilePath];
    }
